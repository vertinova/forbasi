<?php
/**
 * API Realtime Akun Jakarta - FORBASI v3.0
 * 
 * API realtime untuk autentikasi & sinkronisasi akun users, pengcab, dan pengda
 * di wilayah Pengda DKI Jakarta. Password diverifikasi langsung ke database,
...
// KONFIGURASI
// ============================================================
define('API_KEY', 'fbsi-jakarta-2026-S3cur3K3y!');
define('API_VERSION', '3.0');
define('PENGDA_REGION', 'DKI Jakarta');
define('PROVINCE_ID_JAKARTA', 11);
...
switch ($action) {
    ...
    default:
        json_response([
            'success' => true,
            'message' => 'FORBASI Jakarta Realtime API',
            'version' => API_VERSION,
            'region'  => PENGDA_REGION,
            'endpoints' => [
                'POST ?action=login'                 => 'Login realtime (body: username, password). Semua role: User, Pengcab, Pengda.',
                'GET  ?action=accounts'              => 'Daftar semua akun Jakarta (User + Pengcab + Pengda)',
...
function handle_login($conn) {
    ...
    $province_id = PROVINCE_ID_JAKARTA;
    ...
    if ($result->num_rows === 0) {
        json_error('Akun tidak ditemukan di wilayah Pengda DKI Jakarta.', 401);
    }
...
function handle_account($conn) {
    ...
    if ($result->num_rows === 0) {
        json_error('Akun tidak ditemukan di wilayah DKI Jakarta.', 404);
    }
