<?php
/**
 * API Realtime Akun Kalbar - FORBASI v3.0
 * 
 * API realtime untuk autentikasi & sinkronisasi akun users, pengcab, dan pengda
 * di wilayah Pengda Kalimantan Barat. Password diverifikasi langsung ke database,
...
// KONFIGURASI
// ============================================================
define('API_KEY', 'fbsi-kalbar-2026-S3cur3K3y!');
define('API_VERSION', '3.0');
define('PENGDA_REGION', 'Kalimantan Barat');
define('PROVINCE_ID_KALBAR', 20);
...
switch ($action) {
    ...
    default:
        json_response([
            'success' => true,
            'message' => 'FORBASI Kalbar Realtime API',
            'version' => API_VERSION,
            'region'  => PENGDA_REGION,
            'endpoints' => [
                'POST ?action=login'                 => 'Login realtime (body: username, password). Semua role: User, Pengcab, Pengda.',
                'GET  ?action=accounts'              => 'Daftar semua akun Kalbar (User + Pengcab + Pengda)',
...
function handle_login($conn) {
    ...
    $province_id = PROVINCE_ID_KALBAR;
    ...
    if ($result->num_rows === 0) {
        json_error('Akun tidak ditemukan di wilayah Pengda Kalimantan Barat.', 401);
    }
...
function handle_account($conn) {
    ...
    if ($result->num_rows === 0) {
        json_error('Akun tidak ditemukan di wilayah Kalimantan Barat.', 404);
    }
