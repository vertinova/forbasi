...
// Legacy SectionCard styled component
...
const SECTIONS = [
  { key: 'kta',        label: 'Pengajuan KTA',  icon: 'fa-file-alt',      badge: null },
  { key: 'kta_terbit', label: 'KTA Terbit',     icon: 'fa-id-card',       badge: null },
  { key: 'keuangan',   label: 'Keuangan',       icon: 'fa-wallet',        badge: null },
  { key: 'transaksi',  label: 'Transaksi',      icon: 'fa-exchange-alt',  badge: null },
  { key: 'pengguna',   label: 'Pengguna',       icon: 'fa-users',         badge: null },
  { key: 'log',        label: 'Log Aktivitas',  icon: 'fa-history',       badge: null },
  { key: 'profil',     label: 'Profil',         icon: 'fa-user-cog',      badge: null },
];
...
export default function PbDashboard() {
  ...
  }, [filterProvince, filterCity, search]);
  ...
  // reload saldo when filters change
  useEffect(() => {
    if (activeSection === 'keuangan') fetchSaldo();
  }, [saldoFilterMonth, saldoFilterYear]);
  ...
  const menuItems = [
    ...
    { label: 'KTA Terbit',      icon: <i className="fas fa-id-card" />,         onClick: () => setActiveSection('kta_terbit'), active: activeSection === 'kta_terbit' },
  ...
  const renderKtaTerbitSection = () => (
    <SectionCard title="Daftar KTA Terbit" icon="fa-id-card"
      actions={
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative' }}>
            <i className="fas fa-search" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#94a3b8', fontSize: '0.8rem' }} />
            <input type="text" placeholder="Cari klub/barcode..." value={search}
              onChange={e => setSearch(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') fetchIssuedKtas(1); }}
              style={{ padding: '0.45rem 0.75rem 0.45rem 2rem', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: '0.85rem', outline: 'none', width: 200 }} />
          </div>
          <button onClick={() => fetchIssuedKtas(1)}
    ...
    >
      {issuedKtaLoading ? <LoadingSpinner /> : issuedKtas.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>
          <i className="fas fa-id-card" style={{ fontSize: '2.5rem', display: 'block', marginBottom: '0.75rem' }} />
          <p style={{ margin: 0, fontSize: '0.9rem' }}>Belum ada KTA yang terbit</p>
        </div>
      ) : (
        <>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['#', 'Klub', 'Ketua', 'Barcode ID', 'Provinsi / Kota', 'Tanggal Terbit', 'Status Bayar', 'Aksi'].map(h => (
                    <th key={h} style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.75rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {issuedKtas.map((kta, idx) => (
                  <tr key={kta.id}
                    ...
                    onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'}
                    onMouseLeave={e => e.currentTarget.style.background = ''}>
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.8rem', color: '#94a3b8', fontWeight: 600 }}>
                      {(issuedKtaPagination.page - 1) * 20 + idx + 1}
                    </td>
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 34, height: 34, borderRadius: 8, flexShrink: 0, background: `linear-gradient(135deg,${G},${NAVY})`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '0.8rem', fontWeight: 700 }}>
                          {(kta.club_name || '?')[0].toUpperCase()}
                        </div>
                        <span style={{ fontSize: '0.87rem', fontWeight: 600, color: '#1e293b' }}>{kta.club_name}</span>
                      </div>
                    </td>
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.85rem', color: '#475569' }}>{kta.leader_name || '-'}</td>
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <code style={{ fontSize: '0.75rem', background: '#f1f5f9', padding: '3px 8px', borderRadius: 6, color: '#6366f1' }}>
                        {kta.kta_barcode_unique_id || '-'}
                      </code>
                    </td>
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.82rem', color: '#64748b' }}>
                      {[kta.province_name, kta.city_name].filter(Boolean).join(' / ') || '-'}
                    </td>
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.82rem', color: '#94a3b8', whiteSpace: 'nowrap' }}>
                      {kta.kta_issued_at ? new Date(kta.kta_issued_at).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}
                    </td>
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        <span style={{
                          padding: '2px 8px', borderRadius: 12, fontSize: '0.7rem', fontWeight: 600,
                          background: kta.amount_pb_to_pengda ? '#d1fae5' : '#fef3c7',
                          color: kta.amount_pb_to_pengda ? '#059669' : '#d97706'
                        }}>
                          PD: {kta.amount_pb_to_pengda ? '✓' : '–'}
                        </span>
                        <span style={{
                          padding: '2px 8px', borderRadius: 12, fontSize: '0.7rem', fontWeight: 600,
                          background: kta.amount_pb_to_pengcab ? '#d1fae5' : '#fef3c7',
                          color: kta.amount_pb_to_pengcab ? '#059669' : '#d97706'
                        }}>
                          PC: {kta.amount_pb_to_pengcab ? '✓' : '–'}
                        </span>
                      </div>
                    </td>
                    <td style={{ padding: '0.75rem 1rem' }}>
                      <div style={{ display: 'flex', gap: '0.4rem' }}>
                        <Link to={`/pb/kta/${kta.id}`}
                          style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: '#dbeafe', color: '#2563eb', textDecoration: 'none', fontSize: '0.8rem' }}
                          title="Detail">
                          <i className="fas fa-eye" />
                        </Link>
                        {kta.generated_kta_file_path_pb && (
                          <a href={`${import.meta.env.VITE_API_URL?.replace('/api', '')}/uploads/generated_kta_pb/${kta.generated_kta_file_path_pb}`}
          ...
          {/* Pagination */}
          {issuedKtaPagination.totalPages > 1 && (
            <div style={{ padding: '0.75rem 1.5rem', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>
                Halaman <strong>{issuedKtaPagination.page}</strong> dari <strong>{issuedKtaPagination.totalPages}</strong> ({issuedKtaPagination.total} KTA)
              </span>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button disabled={issuedKtaPagination.page <= 1} onClick={() => fetchIssuedKtas(issuedKtaPagination.page - 1)}
                  style={{ padding: '0.4rem 0.75rem', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: issuedKtaPagination.page <= 1 ? 'not-allowed' : 'pointer', fontSize: '0.8rem', color: '#64748b', opacity: issuedKtaPagination.page <= 1 ? 0.5 : 1 }}>
                  ← Prev
                </button>
                <button disabled={issuedKtaPagination.page >= issuedKtaPagination.totalPages} onClick={() => fetchIssuedKtas(issuedKtaPagination.page + 1)}
                  style={{ padding: '0.4rem 0.75rem', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: issuedKtaPagination.page >= issuedKtaPagination.totalPages ? 'not-allowed' : 'pointer', fontSize: '0.8rem', color: '#64748b', opacity: issuedKtaPagination.page >= issuedKtaPagination.totalPages ? 0.5 : 1 }}>
                  Next →
                </button>
              </div>
            </div>
  ...
  const renderKeuanganSection = () => {
    ...
    return (
      <>
        ...
        <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', padding: '1rem 1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <i className="fas fa-filter" style={{ color: '#64748b', fontSize: '0.9rem' }} />
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#475569' }}>Filter:</span>
          </div>
          <select value={saldoFilterMonth} onChange={e => setSaldoFilterMonth(e.target.value)}
            style={{ padding: '0.45rem 0.75rem', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: '0.85rem', minWidth: 130 }}>
            <option value="">Semua Bulan</option>
            {Array.from({ length: 12 }, (_, i) => (
              <option key={i + 1} value={i + 1}>{new Date(0, i).toLocaleString('id-ID', { month: 'long' })}</option>
            ))}
          </select>
          <select value={saldoFilterYear} onChange={e => setSaldoFilterYear(e.target.value)}
            style={{ padding: '0.45rem 0.75rem', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: '0.85rem', minWidth: 100 }}>
            {Array.from({ length: 5 }, (_, i) => {
              const year = new Date().getFullYear() - 2 + i;
              return <option key={year} value={year}>{year}</option>;
            })}
          </select>
          <button onClick={() => { setSaldoFilterMonth(''); setSaldoFilterYear(String(new Date().getFullYear())); }}
            style={{ padding: '0.45rem 0.85rem', borderRadius: 8, border: '1px solid #e2e8f0', background: '#f8fafc', fontSize: '0.82rem', cursor: 'pointer', color: '#64748b' }}>
            Reset
          </button>
        </div>
        ...
        {/* Hutang (Unpaid Amounts) Section */}
        {unpaidAmounts && !saldoLoading && (
          <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', padding: '1.25rem 1.5rem', marginBottom: '1.5rem' }}>
            <h3 style={{ margin: '0 0 1rem', fontSize: '0.95rem', fontWeight: 700, color: '#1e293b', display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ width: 32, height: 32, borderRadius: 8, background: 'linear-gradient(135deg,#ef4444,#b91c1c)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                <i className="fas fa-hand-holding-usd" style={{ color: '#fff', fontSize: '0.8rem' }} />
              </span>
              Hutang yang Belum Dibayar
            </h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: '0.75rem' }}>
              {[
                { label: 'Belum Bayar ke Pengda', amount: unpaidAmounts.hutang_pengda, count: unpaidAmounts.unpaid_kta_count_pengda, rate: 35000, color: '#6366f1', bg: 'linear-gradient(135deg,#ede9fe,#c7d2fe)' },
                { label: 'Belum Bayar ke Pengcab', amount: unpaidAmounts.hutang_pengcab, count: unpaidAmounts.unpaid_kta_count_pengcab, rate: 50000, color: '#3b82f6', bg: 'linear-gradient(135deg,#dbeafe,#bfdbfe)' },
              ].map(h => (
                <div key={h.label} style={{ background: h.bg, borderRadius: 12, padding: '1rem 1.25rem', borderLeft: `4px solid ${h.color}` }}>
                  <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600 }}>{h.label}</div>
                  <div style={{ fontSize: '1.4rem', fontWeight: 800, color: h.color, marginTop: 6 }}>{fmtRp(h.amount)}</div>
                  <div style={{ fontSize: '0.72rem', color: '#94a3b8', marginTop: 4 }}>
                    <i className="fas fa-file-alt" style={{ marginRight: 4 }} />
                    {h.count} KTA × {fmtRp(h.rate)}
                  </div>
                </div>
              ))}
            </div>
            <div style={{ marginTop: '1rem', padding: '0.75rem', background: '#f8fafc', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem' }}>
              <div style={{ fontSize: '0.85rem', color: '#475569' }}>
                <i className="fas fa-chart-bar" style={{ marginRight: 6, color: '#10b981' }} />
                Total KTA Terbit: <strong>{unpaidAmounts.total_issued_kta}</strong>
              </div>
              <div style={{ display: 'flex', gap: '1rem', fontSize: '0.8rem', color: '#64748b' }}>
                <span><i className="fas fa-code" style={{ marginRight: 4, color: '#f59e0b' }} />Developer: {fmtRp(unpaidAmounts.hutang_developer)}</span>
                <span><i className="fas fa-building" style={{ marginRight: 4, color: '#10b981' }} />PB Net: {fmtRp(unpaidAmounts.pb_net)}</span>
              </div>
            </div>
          </div>
        )}
